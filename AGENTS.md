...
* **元数据**：name 必须 `@dsh-sparrow/<插件名>`；description 英文；`repository` 必填（npm `--provenance` 校验）；`files` 清单齐备；README/CHANGELOG 中英双份顶部互链；CHANGELOG 条目按发布顺序从上到下、稳定版覆盖 alpha 全部用户可感知改动、只记真实发布过的版本（发布前的改名等内部历史记 docs/spec）；插件截图统一放 `plugins/<插件>/docs/images/<插件名>.png`（单一来源）；README 一律用**绝对 URL**引用（`https://raw.githubusercontent.com/peiyucn/dsh-sparrow/main/plugins/<插件>/docs/images/<插件名>.png`，GitHub 与 npm 页双端可用，URL 绑定 main 分支）；**图片不打包进 npm 包**（`files` 不含 docs/images，包内无需携带 README 图）
...
* `publish.yml`：push `<插件名>-vX.Y.Z` tag 或 workflow_dispatch 指定插件；解析插件/校验版本/verify 后 npm publish（版本含 `-` 发 next，正式版发 latest）；成功后自动建 GitHub Release；publish job 挂 `environment: npm-publish`（Deployments 留发布记录）；**无 release-control**（通道变更一律发新版本号；deprecate 由 owner 本机手动执行）
