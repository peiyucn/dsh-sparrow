...
* **官方版本跟随**：日常跟随官方 alpha 线（dev 即 alpha 轨；alpha/beta 走 next、rc/稳定走 latest）；**不回头适配 rc.2**（官方已断代弃线）；官方每发新版本做一次对齐/适配，版本级影响预判记 `docs/upstream/`（当前：0.1.3 SessionHandle 变更，见该目录）；**dsh 升级 = 正式适配任务**（先看官方 release note 与社区迁移地图 → 影响清单 → bump 依赖 → typecheck → 修 → verify → 发新版），launcher 的 Update dsh 不随手点、checkout 不随手 pull
