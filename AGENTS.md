...
* **流程**：改动 push dev + 插件 verify → 版本号 + CHANGELOG 双份 → 再 verify + `git diff --check` → 合并 dev→main（fast-forward）并 push → `git tag -a <插件名>-vX.Y.Z -m "<一句话中文说明>"`（必须 -a）→ push tag 自动发布（解析插件、校验 tag==package.json version、verify、`npm publish --access public --provenance --tag next|latest`、自动建 GitHub Release：说明拼两份 CHANGELOG，预发布版本（版本号含 `-`）标 prerelease，**所有版本一律不设 Latest**——每包权威指针是 npm dist-tag）→ `gh run watch` 盯 success + `npm view` 复核版本与 dist-tag → 切回 dev
* **tag 兜底**：push tag 后 30 秒内无对应 Publish run，改 `gh workflow run publish.yml -f plugin=<插件名>` 手动派发（发布内容与 tag 触发一致，仅跳过 Release——用 `gh release create <tag> ${PRERELEASE_FLAG} --latest=false --notes-file <说明>` 补建）
...
* `publish.yml`：push `<插件名>-vX.Y.Z` tag 或 workflow_dispatch 指定插件；解析插件/校验版本/verify 后 npm publish（版本含 `-` 发 next，正式版发 latest）；成功后自动建 GitHub Release（预发布标 prerelease、一律不设 Latest）；publish job 挂 `environment: npm-publish`（Deployments 留发布记录）；**无 release-control**（通道变更一律发新版本号；deprecate 由 owner 本机手动执行）
