...
* **会话事件读取（2026-09-01 适配）**：dsh 0.1.2-alpha.4 移除 `Session.events`（→ 按需读取 API `snapshotEvents()` 等）。`readSessionEvents`（vision.ts）运行时 feature-detect（新 API 优先、旧 `events` 回退）；host 三处读取（currentMainModel 主路由、vision_read 的附件反查与主路由防御检查）均经此助手。适配版本：dsh ≥ 0.1.2-alpha.4（0.1.1-rc.2 起回退兼容）。
