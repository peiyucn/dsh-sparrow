...
2. **备份：移动会话目录** 到插件备份夹（默认 `$DSH_HOME/sessions-archived-backup/`）；**删除：`rm` 会话目录**；只对受检访问器 `assertSessionLocationApi(...).locate(meta)` 返回 `kind: 'jsonl'` 的已知单会话目录执行；
