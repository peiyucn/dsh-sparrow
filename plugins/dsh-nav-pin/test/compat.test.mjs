...
import { cssSupports, missingCapabilities, warnMissingCapabilities, warnUser } from '../lib/compat.js'
...
describe('compat 宿主兼容自检（纯能力版，客户端惰性停用）', () => {
  ...
  it('能力齐备 应该 返回 true 且不告警', (t) => {
    ...
    assert.equal(warnMissingCapabilities(ctx, 'dsh-x', [{ name: 'selector(:has())', ok: true }]), true)
  ...
  it('能力缺失 应该 告警（logger + console 同一句）并返回 false —— 不抛错', (t) => {
    ...
    let ok
    assert.doesNotThrow(() => {
      ok = warnMissingCapabilities(ctx, 'dsh-x', [{ name: 'selector(:has())', ok: false }])
    })
    assert.equal(ok, false)
    assert.equal(warns.length, 1)
    assert.equal(printed.length, 1)
    assert.equal(printed[0], warns[0], 'console 与 logger 必须是同一句话')
    assert.match(warns[0], /selector\(:has\(\)\)/u)
  ...
  it('warnUser 应该 在 ctx.logger 缺失时仍然告警（console 是无条件通道）', (t) => {
    const printed = []
    t.mock.method(console, 'warn', message => { printed.push(message) })
    assert.doesNotThrow(() => { warnUser({}, 'dsh-x: 缺特性') })
    assert.deepEqual(printed, ['dsh-x: 缺特性'])
  })
})
...
describe('compat 能力探针', () => {
  it('cssSupports 应该 在无 CSS.supports 的环境下返回 false 而不是崩', () => {
    // Node 运行环境没有 CSS 全局
    assert.equal(cssSupports('container-type: inline-size'), false)
  })
})
