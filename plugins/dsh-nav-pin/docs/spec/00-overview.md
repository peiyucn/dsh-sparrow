...
* 会话内容最大宽度钳制：拖宽上限从官方「每侧 88px 留白」收紧到 140px，右侧拖拽条不再挤占轮次导航命中区（官方拖拽条 z-index 8 会压过导航 z-index 6）。
...
* 会话区拖到最宽时内容右缘仍留 ≥140px 空白；拖拽条与轮次导航命中区无重叠；未拖动时（自适应宽度）行为与官方一致；输入卡片与内容列同宽轴收缩。
