...
* 会话内容最大宽度钳制：宽列下拖宽上限从官方「每侧 88px 留白」收紧到 120px；窄列下内容压到官方最小宽度 640px 让出导航余量。右侧拖拽条不再挤占轮次导航命中区（官方拖拽条 z-index 8 会压过导航 z-index 6）。
...
* 宽列下会话区拖到最宽时内容右缘仍留 ≥120px 空白，拖拽条与轮次导航命中区无重叠；窄列（对话列 <1000px）下内容压到官方最小宽度 640px；未拖动时宽列行为与官方一致，窄列默认内容 680→640；输入卡片与内容列同宽轴收缩。
