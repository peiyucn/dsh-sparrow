...
Large images pasted into DSH are auto-uploaded to the DeepSeek Files API, but there is no official management UI. This plugin adds a "Files" entry to the sidebar and lists every cloud file under your API key: cursor pagination, upload/expiry times, sizes, single-file deletion, and one-click file_id copy. It reuses the official `DeepSeekFilesClient` — no extra credentials.
...
* **Entry**: the "Files" button at the sidebar footer
