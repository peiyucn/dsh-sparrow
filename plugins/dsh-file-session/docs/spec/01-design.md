...
* **连接事实解析**：官方 adapter 每请求解析 `{ baseURL, apiKey }`（`adapter.ts`：设置节 `llm-deepseek` 的 `baseURL` / `apiKeyEnv` → `ctx.credentials` 解析，缺省回退 $DEEPSEEK_BASE_URL / DEEPSEEK_API_KEY 环境）。插件 host 侧照同一路径解析（正路 seam：`ctx.settings` + `ctx.credentials`），与 chat-suggest 直连模式一致。
...
* **客户端入口**：`sidebar.footer.action` 公开 slot（`ui-sidebar/src/client/contract/slots.ts`；archive-session 已用同一入口）。

## 架构（对齐 dsh-archive-session，完全无状态）

* **host half**：REST 路由，内部用官方 `DeepSeekFilesClient`；插件零本地持久化（无 sidecar、无任务表）。
  * `GET /api/file-session/list?after&limit&order` → 单页（转发官方 list，透传游标）
  * `DELETE /api/file-session/:id` → 删除单条（转发官方 delete）
