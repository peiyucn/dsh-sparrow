...
DSH 贴图的大图会自动上传到 DeepSeek Files API（云端文件库），但官方没有管理界面。本插件在侧边栏加一个「文件」入口，列出你 API key 下的全部云端文件：游标翻页、上传 / 到期时间、大小，支持删除单条与一键复制 file_id。完全复用官方 `DeepSeekFilesClient`，不新增任何凭据。
