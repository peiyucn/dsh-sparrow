...
与官方「轻归档」配合，集中管理已归档的会话：在工作区工具栏加一个入口按钮，点开弹窗列出所有归档会话，每行提供**三档操作**——轻量标题（`@` 仍可拉回）、备份（`@` 不可达，可逆）、删除（`@` 不可达，不可逆）。与官方「轻归档」互补，不替换官方归档菜单。
...
* 工作区工具栏一个入口按钮 + 弹窗列出归档会话（含 archived / running / current 状态）。
* 每行三档操作：
