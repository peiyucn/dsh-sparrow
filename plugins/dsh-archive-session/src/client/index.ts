...
const LOCALE_DICTS = {
  zh: {
    ...
    'state.running': '运行中',
    'state.live': '已打开',
    'state.liveActionHint': '该会话仍处于打开状态，dsh 运行期间无法安全卸载：只能在下次启动 dsh 后备份/删除',
    'state.backendUnsupported': '后端不支持文件级操作',
    'group.live': '打开中（{count}）',
    'group.liveHint': '这些会话已被 dsh 归档，但打开中的会话只能在下次启动 dsh 后才能备份或删除。',
  ...
  en: {
    ...
    'state.live': 'Open',
    'state.liveActionHint': 'This session is still open and cannot be safely unloaded while dsh is running: back it up or delete it after the next dsh startup',
    'state.backendUnsupported': 'Backend does not support file operations',
    'group.live': 'Open ({count})',
    'group.liveHint': 'These sessions are archived but still open in this dsh run. They can only be backed up or deleted after the next dsh startup.',
