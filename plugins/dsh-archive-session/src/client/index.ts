...
const LOCALE_DICTS = {
  zh: {
    ...
    'state.running': '运行中',
    'state.live': '已打开',
    'state.liveActionHint': '该会话仍处于打开状态：请先切换到其他会话、关闭它的窗口，或在侧边栏「进行中」列表结束它，再备份/删除（无需重启 dsh）',
  ...
  en: {
    ...
    'state.live': 'Open',
    'state.liveActionHint': 'This session is still open: switch to another session, close its window, or end it in the in-progress list before backup/delete (no dsh restart needed)',
