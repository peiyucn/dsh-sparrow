...
与官方「轻归档」互补：官方把会话从侧边栏藏起来；本插件在**侧边栏 footer 动作区**加一个入口按钮（查证结论：`ui-workspace` 工具栏没有可注入 slot，故采用公开 slot `sidebar.footer.action`），点开弹窗分**归档区 / 备份区**管理会话——

* **备份（`@` 不可达，可逆）**：把会话目录移到插件备份夹，`@` / 列表不再出现；可移回恢复。
* **删除（`@` 不可达，不可逆）**：`rm` 会话目录，不可恢复；不可逆警示 + 输入完整标题强确认。
* **未释放会话分组**：本次 dsh 运行中驻留（未释放）的会话无法安全移动文件，归档区内分组前置、备份/删除置灰，下次启动 dsh 后可操作。
