...
* [x] 路由实现：`webServer.register`（exact `/api/prefix-completion/complete`）+ 会话校验（`sessions.get`，未命中即拒）+ 凭据（`credentials.resolve(credentialRef(...))`）
...
* 独立 npm 包 `dsh-prefix-completion`，按合集《发布》流程走 tag 或 `npm publish`；官方原生支持输入框续写联想后退役。
