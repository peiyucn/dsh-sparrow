...
- 会话与单轮积分**重启 DSH 后不再清零**，重新打开旧会话也仍显示完整累计。
- 积分弹层宽度缩小。
- 积分弹层右上角的用户徽章点击打开 CodeBuddy 个人主页。

## 0.1.5-rc.2.1（2026-09-11）

- 会话积分总结改成输入框下方官方统计行里的同款胶囊（品牌标 + 「积分 N · M 次调用」）——尺寸、配色、悬停与官方两颗一致，点开显示调用次数与按模型聚合的明细；本会话没有 CodeBuddy 调用时依旧不注入。
- 设置卡、额度卡与模型选择器现在读同一份模型目录，点「获取可用模型」三处同时刷新。
- 保存 Key 时先取账号上下文再拉模型目录，首次目录请求即带上正确的企业头。
- 积分系数刚好不到一百万时不再显示成 `1000K`，而是 `1M`。
- provider 请求不再可能永久悬挂：状态、保存、重扫、配额与回合用量查询都有超时兜底。
- 更换或清空 API Key 后，已经在飞的旧刷新不会再回写旧账号或旧模型清单。
...
- 模型名不再带积分系数后缀（零系数不再显示 `free`）；设置清单、模型选择器与额度卡统一为「模型名 ＋ 右侧只读事实」（`x0.00 · 1M`，即积分系数 · 上下文长度）。
