...
打字停顿片刻，输入框上方会以官方 @ 候选菜单同款悬浮卡给出「接下来可能写的文字」建议：**Tab** 采用进草稿，**Esc** 丢弃。补全由 DeepSeek 官方FIM 补全（Beta）驱动，续写直接站在你的角度接话。
...
* **触发灵敏度**：胶囊右侧 ▾ 可选**高 / 中 / 低**三档（按钮内竖排三点指示当前档：恒显 3 点、自下而上点亮 3/2/1 个，悬停有提示），续写模型固定 `deepseek-v4-flash`。三档规则：
  ...
  |---|---|---|---|---|---|
  | 高 | 250ms | 中文 6 字 / 英文 4 字符 | 联想 | 联想 | 联想 |
  | 中（默认） | 400ms | 中文 10 字 / 英文 6 字符 | 不联想 | 联想 | 不联想 |
  | 低 | 800ms | 中文 14 字 / 英文 10 字符 | 不联想 | 不联想 | 不联想 |
