...
* 2026-08-30 晚模型三档退役、引入**触发灵敏度三档**（高/中/低，= eager/standard/conservative）：flash 足够，▾ 弹层改为灵敏度选择（按钮内竖排三点指示：恒显 3 点、自下而上点亮 3/2/1 个、tooltip 随档位变化），参数表 `TRIGGER_SENSITIVITIES`（停顿时长/最短草稿/夹入英文半词/词后空格/句末标点），持久化键 `dsh-chat-suggest:sensitivity`；规则全量明示在 README 与 docs/spec/04-sensitivity.md。2026-08-31 用户拍板**暂切 pro 实测对比**（客户端 `suggestModelMode: 'pro'`；质量上限更高、单价约 3 倍），结论出来后定档。
