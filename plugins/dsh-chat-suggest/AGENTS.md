...
* 2026-08-30 晚触发门控改**内容自适应通用规则**（`shouldTriggerSuggest` 无语言参数）：草稿含 CJK → 8 字符门槛；纯拉丁草稿 → 4 字符门槛 + 放行单词中间（英文停顿几乎总在词中）。尾随空格两种语境都放行（英文词后预测下一个词、中文空格分词续写）；句末标点、夹入英文半词是否触发随灵敏度伸缩（见下条）。不做 zh/en 硬切换，各语言体验一致。2026-08-31 两次调档记录：用户先拍板**三档整体调钝**（CJK 高 6/中 10/低 14；拉丁高 4/中 6/低 10），切回 FIM 后质量回升，再拍板**回调 2 字**（CJK 高 4/中 8/低 12；拉丁高 2/中 4/低 8）——即当前值。
...
* 2026-08-31 **切回 FIM 补全**（用户拍板「方式错了」）：三方案直连 A/B（正常上下文、4 草稿 ×2 采样）——A-FIM 转写体 **8/8 干净用户口吻**（d1「单项的校验上」、d2「可行吗？」、d3「 fix it」、d4「es fix it / es write in chinese」）；B-伪前缀（「用户：」塞进 assistant 消息）约半数漂移成助手口吻/复读；C-官方语义前缀（system 指令 + 裸草稿）半对半漂。根因：官方 prefix 语义是「续写 assistant 自己的消息」，塞「用户：」进去是与机制对抗，结构性角色漂移。FIM 时代的 plea 失败复现于**无实质历史的寒暄上下文**（哈喽啊），正常上下文 FIM 稳定（半词补全 es fix it 即实测样本），角色漂移/复读由 `cleanSuggestion` + 护栏兜底。实现：`buildFimPrompt`（说话人转写体）+ `/completions`；前缀接口代码已删（git 历史可查）。
* 2026-08-31 续写语言**跟随草稿内容**（`detectDraftLanguage`：草稿含 CJK → zh 标签，否则 en 标签）：实测界面语言 en + 草稿 please 时历史以中文为主，模型续出中文——语言框架必须与草稿一致；客户端不再传 locale 字段。
