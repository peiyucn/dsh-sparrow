...
* 2026-08-31 **切回 FIM 补全**（用户拍板「方式错了」）：三方案直连 A/B（正常上下文、4 草稿 ×2 采样）——A-FIM 转写体 **8/8 干净用户口吻**（d1「单项的校验上」、d2「可行吗？」、d3「 fix it」、d4「es fix it / es write in chinese」）；B-伪前缀（「用户：」塞进 assistant 消息）约半数漂移成助手口吻/复读；C-官方语义前缀（system 指令 + 裸草稿）半对半漂。根因：官方 prefix 语义是「续写 assistant 自己的消息」，塞「用户：」进去是与机制对抗，结构性角色漂移。FIM 时代的 plea 失败复现于**无实质历史的寒暄上下文**（哈喽啊），正常上下文 FIM 稳定；且当前标准档拉丁门槛 6 已让该场景不再触发，角色漂移/复读由 `cleanSuggestion` + 护栏兜底。实现：`buildFimPrompt`（说话人转写体）+ `/completions`；前缀接口代码已删（git 历史可查）。
