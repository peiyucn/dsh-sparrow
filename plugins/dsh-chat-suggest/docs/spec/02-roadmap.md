...
* [x] 路由实现：`webServer.register`（exact `/api/chat-suggest/complete`）+ 会话校验（`sessions.get`，未命中即拒）+ 凭据（`credentials.resolve(credentialRef(...))`）
...
* [x] 触发形态门控 `shouldTriggerSuggest`（句末/单词中间/尾随空白/过短不触发）
...
* 独立 npm 包 `dsh-chat-suggest`，按合集《发布》流程走 tag 或 `npm publish`；官方原生支持输入框续写联想后退役。
