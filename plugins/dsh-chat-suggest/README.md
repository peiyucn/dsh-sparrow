...
> Do **not** run `npm install @dsh-sparrow/dsh-chat-suggest` directly — that only downloads the package into a `node_modules` and does not register it in the DSH web profile. Install with the `dsh plugin` command above, then restart DSH.
...
* **Sensitivity**: the "dots + ▾" zone on the right side of the pill picks **High / Medium / Low** (separated by a divider; clicking the whole zone opens the level menu without toggling the switch — 3 square dots always shown, 3/2/1 lit from the bottom, with a hover hint); the completion model **follows your selected main model** (v4-pro / v4-flash; falls back to pro for vision etc.). The rules:
