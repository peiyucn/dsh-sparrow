...
> **分组标题的定案（2026-09-25 第十/十一轮；完整决策记录见
> [`docs/upstream/0.1.7/适配完成报告.md`](../../../docs/upstream/0.1.7/适配完成报告.md) 的「第十轮」「第十一轮」）**：
...
> `border-radius` = `--dsw-radius-lg`（**同心圆角** = 菜单圆角 − 内边距，且正是官方菜单
> **自己**那个圆角 token —— owner：「和菜单的圆角一致」）。
...
> **切圆治的是下沿那道横贯全宽的直边** —— 上沿两角本来就被菜单的 `overflow: hidden`
> 切掉了，所以它才是"看着像一条带"的来源。
