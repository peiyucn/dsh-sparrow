...
> **分组标题的定案（2026-09-25 第十～十二轮；完整决策记录见
> [`docs/upstream/0.1.7/适配完成报告.md`](../../../docs/upstream/0.1.7/适配完成报告.md) 的「第十轮」「第十一轮」「第十二轮」）**：
...
>
> ⚠️ **圆角为什么从标题挪到滚动容器**（第十二轮，owner：「改成圆角后确实边缘会漏」）：
> 第十一轮曾把 `border-radius: --dsw-radius-lg` 切在标题上。那是**几何上必然漏**的写法 ——
> 「标题矩形 − 圆角」那块缺口**真的没画**，而缺口里正好是滚动的行（悬停底铺满整行宽、
> 行文字从 x=8 起），于是行从缺口透出来。真机实测（吸顶条，数条矩形内"漏出的行"像素）：
> 圆角 4px→6、6→16、8→30、16（被 clamp 成 13）→**70**；**方角→0**。
> 附：条高 26px，声明 16px 会被等比压到 13px（半高），那一版其实是个胶囊。
>
> 正解：圆角交给**滚动容器**（`GROUPED_MENU_SCROLLER_SELECTOR` + `GROUPED_MENU_SCROLLER_RADIUS`）。
> 容器顶角的裁剪**同时作用于标题与行** ⇒ 缺口里既没有标题也没有行，露出的是菜单自己的玻璃
> （与卡片同源，不用猜颜色）；标题方角 ⇒ 没有缺口 ⇒ **结构上不可能漏**。实测 6 个滚动位置全 0。
> 半径取**同心值** = 菜单外圆角 − 菜单内边距，而**两个菜单的外圆角不同**
> （官方 `MenuSurface` 16−4=12 = `--dsw-radius-md`；codebuddy 20−4=16 = `--dsw-radius-lg`），
> 所以半径经 `--dsh-theme-tone-menu-inner-radius` 间接取值：规则默认给官方值，
...
> (41.0,42.0,46.0) —— 差 **0.7 级**。也就是说第十一轮的圆角**是靠"缺口里露出的行"
> （比标题亮 18~45 级）才被看见的**；「圆角看得见」与「边缘不漏」在那个写法下互斥。
> 改由容器承担后，圆弧的边界是「标题 / 菜单玻璃」，两者同源 ⇒ 视觉上就是**卡片自己的圆角**，
> 而不是一条带子的角。
>
> ⚠️ **填充为什么必须有**（第十一轮做了决定性实验）：卡片是玻璃，颜色**不是常量** ——
> 在菜单**背后**塞一块白板，同一张卡片从 (53.0,52.9,63.6) 变成 (172.9,172.9,179.4)
> （差 124 级），塞黑板又变成 (48.7,48.1,54.5)；而标题纹丝不动（它不透明、自己画自己）。
> 所以"填一个和卡片完全相同的颜色"在数学上无解（要一样的东西本身在动）；
> "官方原样（只留半透明填充）"则会让滚动的行透上来 —— 每滚动 60px 差分 +23 级，
> 正是 owner 第三轮否掉的那版。唯一能做到**逐像素相同**的是"标题什么都不画 + 不吸顶"
> （实测差 ≤0.2 级），代价是丢掉"滚动时钉住分组名"这个官方行为。
