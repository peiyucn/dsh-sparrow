...
> **来源**：owner 追加需求 —— 「让界面里能弹出来的框都继承咱们选择的色调」。
> 本文替代 `00-overview.md`「不做」里原来那条「不染菜单 / 弹窗」的早期范围决定（见第 8 节）。

## 1) 问题

前三份 spec 处理的都是**地面**：底色、左栏、光晕、颗粒。但用户真正频繁盯着的是**浮起来的东西** ——
下拉菜单、模态对话框、浮动提示条。它们的颜色原来由官方的中性灰阶给（暗轴 `rgb(53,54,56)` 那一档），
...
* **本色**（`ToneSpec.tint`）就是 `bottom` 用的那个颜色 —— 原来它在色调表里写了两遍
  （`bottom: 'rgba(96, 78, 168, .18)'`），现在抽成 `tint: '96, 78, 168'` 一处定义：
  `bottom = rgba(tint, DEPTH_ALPHA[轴])`、`浮层 = color-mix(tint P%, 官方 rung)`。
...
## 4) 两轴的比例为什么差这么多（暗 14% / 浅 5%）

同一套机制，两个轴的地形完全不同：

* **暗轴**：rung 本身是足够亮的中性灰（`rgb(53,54,56)`，三通道和 163），离近黑底（和 36）很远，
  本色可以给得多 —— 14% 就能让菜单读起来是「紫黑 / 酒红 / 墨绿」而不是灰。
* **浅轴**：rung 只有 245–255 这点余量。往浅色里混任何本色都只会**变暗**，比例给大了色相还没出来、
  明度先塌下去。所以浅轴取 5%，只借一点色相 —— **深度由锚点（`bluish-60`）给，不由比例给**。

浅轴这条上界是硬的：只算比例的话，从底色 `#f7fafd`（和 750）到白（765）只有 15 格余量，
本色越饱和能加的比例越小（蓝色理论上限约 6%）。**所以浅轴不要靠「加比例」求存在感** ——
浅轴的深度与色相由**面板档位（`SURFACE_RUNGS.light`）**与**顶光**给。

官方浅色轴的浮层与底色**本来就是同值**（都是白），抬升感靠阴影与描边、不靠填充 ——
所以浅轴的「浮层更亮」本来就不是官方的关系，本插件也不该照抄。

> **浅轴的染色依然淡**（5%）。色调存在感由**锚点的深度 + 浮层表面的光/纵深/颗粒**共同承载，
> 不靠加大比例 —— 比例一大色相还没出来，明度先塌，就是 owner 说的「军大衣」。
...
> owner 口径（三轮）：「弹出来的全是纯色的？」→「**其实弹出框透明的效果可以不要，但是色调、质感、颗粒、金色渐变得有**」
> →「**我觉得弹出框颜色偏浅了，有点变成军大衣的感觉了**」
...
**这一节是兜底，不是主责**：菜单族的主通道已经换成 token（`POPUP_TOKENS`，见 §8.1）——
...
（中途试过「浮层 = 对话区底色」，**owner 已收回该建议**，见 §3。）

* **上下三层**：颗粒压在最上面（才像砂面）→ 顶部光层（共享光源）→ 底部本色辉光（该色调自己的纵深）。
* **浮层用色调表原值，不放大** —— 浮层是**实况**，不是色卡预览。
  （历史上这里写的是「实况永不放大」，那是**旧**口径：现在色卡为了「让 83px 的小卡读出区别」
  才在深色轴放大；而**卡面与实况必须一致** —— 浅色轴已把实况 α 提到卡面等效值，
  见 `PREVIEW_ALPHA_SCALE` 与 03-palette。浮层这条**始终**是原值，不受那次对齐影响。）
...
**排除条件为什么是 `> img` 而不是 `> [aria-hidden]`**：
最初写的是后者，理由是灯箱的遮罩是它的直接子元素。但那个条件**误伤了「token 消耗」两个弹层** ——
`StatsPills` / `TurnUsagePanel` 的 `.panel` 里也各有一个 `<div className={titleRule} aria-hidden>`
当直接子元素（画标题下那条细分隔线）→ 被一起排除，于是它们一直是纯色。
灯箱真正的唯一特征是那枚**整屏 `<img>`**。改用 `> img` 后实测：
...
**曾经没盖到的（owner 反馈后已全部补上）**：工具提示（两轴反色）、
以及 `ui-jobs` 那个**没有 role** 的 `<ul>` 任务列表、子代理血缘弹层。
三条根因都是**锚点表漏了 role**：
...
于是 `body >` 把两个**一起漏掉**，那条规则**一条都命不中**（死规则）→ owner 看到
「官方对话当行 hover 出的框也没有适配样式」。**教训：收窄选择器之前先核实「它到底长在哪」，
别把推断写进注释当依据。**
...
三条症状曾真机出现过：**对话框跟着页面滚动**、**权限/模型下拉有颗粒而其他弹层是纯色**、
「对话框下面那三个胶囊没改」—— 三条根因都在本插件，且都是「CSS 看着对、其实不生效或误伤」，
静态读代码很难发现，故单列：
...
| 玻璃规则给底座写了 `position` | 输入框**跟着内容滚走**（不再钉在底部） | glass 的底座选择器是 `body:not([…]) [data-phase='active'] [data-composer-seat]` = **(0,3,1)**，比官方 `.root[data-phase='active'] .composerSeat` 的 **(0,3,0)** **高** —— 那条「兜底 `position: relative`」（注释自以为不生效）实际**压掉了官方的 `position: sticky`** | 删掉那条 `position`（官方 sticky 本身就是定位元素，`::before` 的包含块已成立）；测试断言底座本体不得出现 `position:` |
...
浮层尺度取中间档：比整屏铺得开（整屏形状在小面上全落在盒外），比色卡收一档。

**但「收一档」当年收得不够 —— 顶光被 owner 打回来了**（55% → **42%**）：
...
「军大衣」的根因就在这里：浅轴的页面光层 α 当时是 `.28`（为整屏存在感调的），同一份浓度铺在 30% 的
面积上就从「一条顶光」变成了「一层奶茶色的色浆」，米色叠绿 = 卡其。**压的是面积，不是浓度** ——
α 一个字没动（那三个数是 owner 定的，见 4.1）。底部染色没动（本来就只有 27%，与整屏 31% 同量级）。

> 浅色轴的光层后来从暖金换成了**主色**（见 03-palette，中间经过冷白 / 银白两版）—— 这进一步缓解了「军大衣」：
> 本色本来就是这个空间的主色，叠在浮层上不会引入外来色相。本节的**几何结论不受影响**。

## 7) 交互态（选中 / hover / 按钮面）

> owner 看着设置页问：「像这些，选中、hover、按钮、下拉菜单、内部元素等的颜色，是不是也得适配一下？」
> 指的正是下面这几族 —— 它们此前都还是官方中性值。
...
**洗染比例比面高一档**（`WASH_TINT` 深 `.25` / 浅 `.15`，面是 `.14` / `.05`）：低 alpha 上色相本来就难读，
不推远一点等于没染。
...
owner 的原话里有「**内部元素**」四个字，这一节就是它。第 6 节给了浮层**外观**（填充 + 颗粒 + 光），
但**框里面**还剩两类纯中性：**分隔线 / 描边**与**滚动条**。
...
owner 圈出输入框左下那两个圆按钮（`+` 命令面板 / 附件）：
「这两个按钮得适配下，我觉得**像下拉菜单一样，默认底就不要了，保留 hover 底就行**。」
...
**顺带核过**：权限按钮（`Access mode`）**本来就是透明的**（真机 `rgba(0,0,0,0)`），
不需要处理；两个模型/权限类触发器同理。
...
| 项 | 值 |
| :--- | :--- |
| **整族收敛成一档**（`SURFACE_RUNGS`） | 暗轴 `875`、浅轴 `60`，`layer-1/2/3 + 菜单 + 提示条` 全部同色 |
...
| `dsh-codebuddy-credits` | 会话积分卡片宽度取 **260px**（`min(260px, calc(100vw - 24px))`） | 官方面板 `min-width` 是 300px，而这张卡是**短标签 + 一个数字**，撑到 300 就空得发慌。headless `max-content` 实测：中文 1 个模型 216.4px / 无模型 176px / 轮次行 186.4px / 超长 id 352.1px → 取 260 既有呼吸感又不空。测试钉住「是 260，既不是官方的 300、也不是试错过的 200」 |

## 9) 待真机调

1. **暗轴比例 `.14`**：偏淡 → `.18`；偏艳 → `.10`。改一个数字，全部色调与全部 rung 同步。
2. **浅轴比例 `.05`**：上界约 `.06`（见第 4 节），能动的不多 —— **浅轴要变深请挪锚点，不要加比例**。
3. **浮层是否「分不出是个弹窗」**：现锚 `bluish-875`（暗）/ `bluish-60`（浅），整族一档。
   要拉开一点就把整族抬一档（暗 `850` / 浅回到 `00`），**别退回官方的三档阶梯**
...
14. **浅灰内嵌面（代码块 / 三张停靠卡）太浅或太重**：`INSET_TINT.light`（现 `.12`）。
    ...
    深色轴同一常量（`.14`）。
15. ~~**底座底部不透带的高度**~~ —— **已整条撤回**（owner 当次选择「都撤了，改回原来的「透」」）：
    那一截保持透，`SEAT_SOLID_PX` 等常量与规则已删除。三版经过见 04-glass §3.2，
    结论是**方向本身错了**（owner 最初说的「穿帮」不是这一截的透明度）。
...
真机复核（`playwright-cli attach --cdp`，端口 `9334`，专用 `--user-data-dir`）：
