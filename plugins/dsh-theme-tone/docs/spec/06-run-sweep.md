...
## 4) ⚠️ `background-attachment: scroll, fixed` 是个真 bug（已修）
...
**同一个 bug 在右边栏那条规则里也存在，一并修掉**（`glass.ts`），
两处都补了守卫测试（禁止 `scroll, fixed`、要求单个 `fixed`）。
...
1. **带子是否真的隐形**：跑个命令让某行进入 running，看背景上有没有一条横带。
   * 隐形 ✓ → 成了；
   * **仍可见** → 说明我们的背景板还差一项（比如 `.root` 那层纯色之上还有别的层）。
