...
/**
 * **顶光收束位置的变量名** —— 两轴取不同的值（其余几何完全相同）。
 *
 * ## 为什么必须按轴分开（owner：「浅色模式上下主色向中间渐变整体比较愣」）
 *
 * 两轴的**几何逐字相同**，只有 alpha 差一个量级（深色轴顶光金 `.09`，浅色轴主色 `.55`）。
 * 同样收在 62%，浅色轴那条「到此为止」的边就明显 6 倍 —— 而收束点之外是**纯底色**：
 * 整屏椭圆 `at 50% -10vh` + `ry 45vh`，62% 收束 → 顶光在 **y ≈ 145px** 就没了，
 * 于是屏高 **18% ~ 84%** 是一大片没有任何渐变的死区。两条硬边夹一片平底 = 「愣」。
 * 深色轴看不出来只是因为 `.09` 太淡。
 *
 * 浅色轴把收束点推到 **100%**（顶光一路淡到中段），同时把顶光 alpha 从 `.55` 压到 `.48`——
 * **同样的「色量」摊到更长距离上**，所以顶部浓度基本不变，变的只是收束的软硬。
 *
 * 值由 `tones.ts` 按轴发（浅色 `100%` / 深色沿用 `62%`），**深色轴一个字节没动**。
 */
