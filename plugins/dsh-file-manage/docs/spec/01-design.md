...
* 分页（数据 / 渲染分离，2026-09 审计后）：数据按 `FILE_PAGE_SIZE`（200）游标拉取，DOM 按渲染窗口 `RENDER_PAGE_SIZE`（20，owner 定案）裁剪——「加载更多」先延伸窗口（不发请求），窗口盖满已加载行且 has_more 时再拉下一页（archive spec 09 同款窗口模式：配额 10000 下 DOM 不随打开次数无界累积，滚底 = 滚到已渲染窗口末尾）。
* 总数统计带 30s TTL 缓存（`COUNT_CACHE_TTL_MS`，删除成功后失效）：面板快速重开/重试不重复「翻到底」全量扫描（count 最多 10 页）。
...
本机 dsh checkout：`~/.dsh-launcher-panel/source`（release/dsh-0.1.2-alpha.3）。开工时记录所适配 dsh 版本与导出行号复核。
