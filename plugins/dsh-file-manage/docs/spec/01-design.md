...
* 分页（数据 / 渲染分离，2026-09 审计后）：数据按 `FILE_PAGE_SIZE`（200）游标拉取，DOM 按渲染窗口 `RENDER_PAGE_SIZE`（100）裁剪——「加载更多」先延伸窗口（不发请求），窗口盖满已加载行且 has_more 时再拉下一页（archive spec 09 同款窗口模式：配额 10000 下 DOM 不随打开次数无界累积，滚底 = 滚到已渲染窗口末尾）。
