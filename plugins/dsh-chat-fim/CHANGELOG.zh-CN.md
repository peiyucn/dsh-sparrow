...
- 修掉刚启动 DSH 时联想开关无视所选模型的问题（此前要刷新一次页面才会按主模型决定显隐）：宿主**还判不出来**时不再当成「支持」，而是等一个结论 —— 开关只在确认该模型支持后出现，或在补查用尽后给出结果，不会凭第一次猜测就显示。
## 0.1.5-rc.2（2026-09-10）

- 版本线对齐官方 dsh 0.1.5-rc.2。
- 在 dsh 0.1.5 上，联想开关与候选菜单恢复显示：上一版因不认识新的会话格式而把它们整体隐藏了。
- 每次请求联想不再拷贝整份会话日志，长会话下响应更稳。
- 联想现在跟随新的 deepseek-flash 主模型（此前不认识它会回退到 deepseek-v4-pro），回退默认也改为 deepseek-flash。
