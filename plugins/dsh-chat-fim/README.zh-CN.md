...
* **触发**：开启后打字停顿自动联想；**句末标点（`。！？.!?;；`）结尾一律不触发**，拼音等输入法组合输入中不触发；其余规则（停顿时长、最短草稿、夹入英文半词、词后空格）随下方灵敏度三档伸缩
* **建议**：Tab 采用、Esc 丢弃（也可直接点选）；官方 @/斜杠候选菜单打开时本建议自动让位
* **触发灵敏度**：胶囊右侧 ▾ 可选**高 / 中 / 低**三档（按钮内 3/2/1 个小方块指示当前档，悬停有提示），续写模型固定 `deepseek-v4-flash`。三档规则：

  | 档位 | 停顿 | 最短草稿 | 夹入英文半词 | 词后空格 |
  |---|---|---|---|---|
  | 高 | 250ms | 中文 4 字 / 英文 2 字符 | 联想 | 联想 |
  | 中（默认） | 400ms | 中文 8 字 / 英文 3 字符 | 不联想 | 联想 |
  | 低 | 800ms | 中文 12 字 / 英文 5 字符 | 不联想 | 不联想 |
