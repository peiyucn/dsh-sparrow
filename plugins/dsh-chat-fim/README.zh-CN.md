...
适配 dsh 0.1.5-rc.1（本版构建与验证所对齐的确切官方版本线；更新的版本线、尤其是预发布版本不在承诺范围内），并需要可用的 `pnpm`（`dsh plugin` 会把安装操作转发给 pnpm）。
...
* **触发灵敏度**：胶囊右侧「方点 + ▾」触发区可选**高 / 中 / 低**三档（点击整区只开选档菜单、不会误触开关；方点恒显 3 个、自下而上点亮 3/2/1 个指示当前档，悬停有提示），续写模型**跟随你选择的主模型**（deepseek-flash / v4-pro / v4-flash；vision 等回退 deepseek-flash）。三档规则：
...
![续写联想建议卡](https://raw.githubusercontent.com/peiyucn/dsh-sparrow/main/resources/dsh-chat-fim.png)
