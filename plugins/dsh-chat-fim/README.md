...
* **续写模型三档可选**（菜单底部选择器，本地记忆）：`自动`（默认，跟随主模型：官方 pro/flash 用之，vision 回退 pro）/ `Pro` / `Flash`（快且省，输出单价为 pro 的 1/3）；菜单右下角显示本次续写的 token 数与实际使用模型；注：官方 API schema 只列 v4-pro，但实测 flash 可用（2026-08-30 直连实测）；
