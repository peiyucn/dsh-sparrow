...
* **续写模型三档可选**（开关胶囊内置 ▾ 箭头，官方下拉按钮同款无边框样式，点箭头开菜单、点胶囊其余位置切换开关，箭头拉出时旋转；选项写全名 `auto` / `deepseek-v4-flash` / `deepseek-v4-pro`；弹层下方空间不足自动向上；本地记忆）：`auto`（默认，跟随主模型：官方 pro/flash 用之，vision 回退 pro）；建议菜单右下角显示本次续写的 tok 数与实际模型、温度（如 `1,234 tok · deepseek-v4-flash · T0.4`）；注：官方 API schema 只列 v4-pro，但实测 flash 可用（2026-08-30 直连实测）；
