...
* 补全模型（2026-08-30 定，2026-09-10 更新）：**跟随主模型**（`resolveSuggestModel`）——主模型为 deepseek-official 的 `deepseek-flash` / v4-pro / v4-flash 时用主模型补全（计费随之），vision-exp / 未知 / 非官方回退配置默认 `model`（现为 `deepseek-flash`）；依据：官方 API schema 只列 v4-pro，但直连实测 flash 与 deepseek-flash 均可（见 README 实测记录）；
...
* **触发**：草稿变更后停顿 ≥ 400ms（客户端常量 `PAUSE_MS`，非插件配置），且**草稿形态门控通过**（`shouldTriggerSuggest`，2026-08-30 实测驱动，同日晚改为**按草稿内容自适应的通用规则**——不做 zh/en 硬切换，各语言体验一致）：
  * trim 后达到最短长度：含 CJK 字符的草稿 ≥ `MIN_TRIGGER_DRAFT_CHARS`（8 字符——FIM 时代实测好值；2026-08-31 曾整体调钝后按用户拍板回调：高 4 / 中 8 / 低 12）；纯拉丁草稿 ≥ `MIN_TRIGGER_DRAFT_CHARS_LATIN`（6 字符——完整单词才触发；半词建议按用户拍板去掉：plea×4 实测全错为 es、ple→as，字母级补全不可靠；高 2 / 低 8），上下文过短不触发；
