...
* 多建议：FIM 接口无 `n` 参数，按 `suggestionCount`（默认 2）并行请求、温度错开采样；`allSettled` 部分失败保留成功建议；
...
* **触发**：草稿变更后停顿 ≥ `TRIGGER_PAUSE_MS`（400ms，可配置），且**草稿形态门控通过**（`shouldTriggerFim`，2026-08-30 实测驱动）：
  * trim 后 ≥ `MIN_TRIGGER_DRAFT_CHARS`（8 字符），上下文过短不触发；
  * 末尾非句末标点（`。！？.!?;；`）——句末已完整，FIM 会续出新一句而不是接话（实测：草稿「还有个问题，fim接口是计费的么？」被续成「claude code好像没有fim呢…」，衔接不上）；
  * 末尾非空白（用户刚敲空格/换行，正在输入）；
  * 不落在 ASCII 单词中间（末尾两字符都是 `[A-Za-z0-9]` = 停在一个单词中间）；中文逐字成词、逗号/顿号后是天然续写点，均放行；
