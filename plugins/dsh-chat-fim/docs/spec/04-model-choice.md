...
* **选择器在开关胶囊内**（2026-08-30 用户要求）：胶囊右侧内置 ▾ 箭头（点击只开菜单、不切换开关，stopPropagation）；弹层锚定胶囊右下，**下方空间不足时自动向上弹出**（官方下拉惯例，估算高度 112px + 8px 边距判定），点外部 / Esc / 滚动关闭；选中项打勾，写 localStorage 并作用于下一次请求；
