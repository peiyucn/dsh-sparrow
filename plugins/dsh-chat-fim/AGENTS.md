...
* 2026-08-30 晚触发门控改**内容自适应通用规则**（`shouldTriggerFim` 无语言参数）：草稿含 CJK → 8 字符门槛 + 抑制夹在中文里的英文单词续一半；纯拉丁草稿 → 3 字符门槛 + 放行单词中间（英文停顿几乎总在词中）。尾随空格两种语境都放行（英文词后预测下一个词、中文空格分词续写）；仅句末标点仍不触发。不做 zh/en 硬切换，各语言体验一致。
* 2026-08-30 晚模型三档退役、引入**触发灵敏度三档**（高/中/低，= eager/standard/conservative）：flash 足够，▾ 弹层改为灵敏度选择（按钮内 3/2/1 小方块指示、tooltip 随档位变化），参数表 `FIM_SENSITIVITIES`（停顿时长/最短草稿/夹入英文半词/词后空格），持久化键 `dsh-chat-fim:sensitivity`；规则全量明示在 README 与 docs/spec/04-sensitivity.md。
